...
function section = initSection(r, c, theta, af, headerLinesIn, Re)
section.r = r;
section.c = c;
section.theta = theta;
section.af = af;
section.headerLinesIn = headerLinesIn;
section.Re = Re;
end
...
station.headerLinesIn = section.headerLinesIn(n);
station.Re = section.Re(n);
end
...
[Cl, Cd] = afFromFile(alpha, station.af, station.headerLinesIn);
...
% msg = ['R(phistar) =' num2str(residual(phistar))]; % test phistar by evaluating R at phistar
% disp(msg);
...
headerLinesIn = 3;
skipNo = zeros(1, length(r)) + headerLinesIn;
re = 50000;

testSection = initSection(r, chord, theta, airfoils, skipNo, re);
...
% [Cl, Cd] = afFromFile(-180, testStation.af, 3)
...
msg = ['At max Cp, Cp = ' num2str(CpMaxCondition(1)), ', TSR = ' num2str(CpMaxCondition(2)), ', Ct = ' num2str(CpMaxCondition(3)), ', Cq = ' num2str(CpMaxCondition(4))];
disp(msg);
msg = ['At max Ct, Ct = ' num2str(CtMaxCondition(1)), ', TSR = ' num2str(CtMaxCondition(2)), ', Cp = ' num2str(CtMaxCondition(3)), ', Cq = ' num2str(CtMaxCondition(4))];
disp(msg);
msg = ['At max Cq, Cq = ' num2str(CqMaxCondition(1)), ', TSR = ' num2str(CqMaxCondition(2)), ', Cp = ' num2str(CqMaxCondition(3)), ', Ct = '  num2str(CqMaxCondition(4))];
disp(msg);